// File: src/services/researchCategory.service.ts
import { prisma } from '../database/prisma';
import { CreateResearchCategoryRequest, UpdateResearchCategoryRequest, ResearchCategoryResponse } from '../types/researchCategory.types';
import { generateSlug, generateUniqueSlug } from '../utils/slug.utils';

export const createCategory = async (data: CreateResearchCategoryRequest): Promise<ResearchCategoryResponse> => {
  ...
  if (!slug) {
    ...
    const existingCategories = await prisma.researchCategory.findMany({
      select: { slug: true }
    });
...
export const getAllCategories = async (params: {
  ...
  const [categories, total] = await Promise.all([
    ...
    prisma.researchCategory.count({ where }),
  ]);
...
export const getCategoryById = async (id: number): Promise<ResearchCategoryResponse | null> => {
...
export const getCategoryBySlug = async (slug: string): Promise<ResearchCategoryResponse | null> => {
  const category = await prisma.researchCategory.findUnique({
    where: { slug },
    include: {
      translations: true,
...
export const updateCategory = async (id: number, data: UpdateResearchCategoryRequest): Promise<ResearchCategoryResponse> => {
  const { translations, ...categoryData } = data;
...
export const checkParentExists = async (parentId: number): Promise<boolean> => {
  const parent = await prisma.researchCategory.findUnique({
    where: { id: parentId },
  });
