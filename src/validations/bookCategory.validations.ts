...
const bookCategoryTranslationSchema = z.object({
...
export const createBookCategorySchema = z.object({
  ...
  translations: z.array(bookCategoryTranslationSchema).min(1),
});
...
export const updateBookCategorySchema = z.object({
  ...
  translations: z.array(bookCategoryTranslationSchema).optional(),
});

export const bookCategoryIdSchema = z.object({
  id: z.string().transform((val) => parseInt(val, 10)).refine((val) => val > 0, 'ID must be a positive number'),
});

export const bookCategorySlugSchema = z.object({
