	...
	if (!translations || !Array.isArray(translations)) {
		...
		translations.forEach((translation, index) => {
			if (
				...
				typeof translation.languageCode !== "string"
			) {
				errors.push(
					`Translation ${index + 1}: languageCode is required`,
				)
			}
			...
			if (typeof translation.isDefault !== "boolean") {
				errors.push(
					`Translation ${index + 1}: isDefault must be a boolean`,
				)
			}
		})
	...
	if (attachments !== undefined) {
		if (!Array.isArray(attachments)) {
			...
			attachments.forEach((attachment, index) => {
				if (
					...
					errors.push(
						`Attachment ${
							index + 1
						}: attachmentId is required and must be a number`,
					)
				}
				...
				if (!attachment.type || typeof attachment.type !== "string") {
					...
					!["image", "attachment", "other"].includes(attachment.type)
				) {
					errors.push(
						`Attachment ${
							index + 1
						}: type must be one of: image, attachment, other`,
					)
				}

				if (
					attachment.order === undefined ||
					typeof attachment.order !== "number"
				) {
					errors.push(
						`Attachment ${
							index + 1
			...
			if (duplicateIds.length > 0) {
				errors.push(
					`Duplicate attachment IDs found: ${duplicateIds.join(
						", ",
					)}`,
				)
			}
			...
			if (duplicateOrders.length > 0) {
				errors.push(
					`Duplicate attachment order values found: ${duplicateOrders.join(
						", ",
					)}`,
				)
			}
...
export function validateUpdateArticle(
	...
	if (translations !== undefined) {
		if (!Array.isArray(translations)) {
			...
			errors.push(
				"At least one translation is required when updating translations",
			)
		} else {
			translations.forEach((translation, index) => {
				if (
				...
				if (
					translation.isDefault !== undefined &&
					typeof translation.isDefault !== "boolean"
				) {
					errors.push(
						`Translation ${index + 1}: isDefault must be a boolean`,
					)
